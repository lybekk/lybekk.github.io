/**
 * replaces markdownFiles.js
 */
const { ContentTransformer } = require('lybekk-tech-content-transformer');

function preparedContent() {
	const contentTransformer = new ContentTransformer()
	const data = contentTransformer.contentBatch
	return data
}
...
const parseMarkdownDir = () => {
  ...
  if (markdownDir) {
    ...
    const files = glob.sync("** / *.md", optionsGlob); temp: spaces added for comment
    for (f of files) {
